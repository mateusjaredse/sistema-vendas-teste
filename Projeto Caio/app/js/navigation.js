...
function syncPainelGeralGrid(){
  ...
  const compact=frameWidth>0&&frameWidth<1180;
  const reportMinWidth=frameWidth<1350?300:360;
  const reportWidth=Math.round(Math.min(520,Math.max(reportMinWidth,frameWidth*.29)));
  const template=compact?"1fr":`minmax(0,1fr) minmax(${reportWidth}px,.42fr)`;
  ...
  if(frame.contentWindow){
    frame.contentWindow.postMessage({type:"painel-geral-layout",template,frameWidth,reportWidth},"*");
  }
}
...
(function(){
  ...
  if(painelFrame&&"ResizeObserver" in window){
    new ResizeObserver(()=>syncPainelGeralGrid()).observe(painelFrame);
  }
