...
function syncResponsiveScale(){
  ...
  const viewportHeight=Math.max(1,Math.round(document.documentElement.clientHeight||window.innerHeight||1080));
  const fullHdScreen=screenWidth>=1700&&screenHeight>=900;
  const compactViewport=viewportWidth<=1500||viewportHeight<=850;
  let nextScale=1;
  if(!fullHdScreen||compactViewport){
    nextScale=Math.min(1,Math.max(minScale,viewportWidth/baseWidth));
  }
  const root=document.documentElement;
  const scaleText=nextScale.toFixed(4);
  const virtualWidth=Math.ceil(viewportWidth/nextScale);
  const virtualHeight=Math.ceil(viewportHeight/nextScale);
  const fontBoost=nextScale<.999 ? (nextScale<.76 ? 2.6 : 1.6) : 0;
  ...
  root.style.setProperty("--app-width",`${virtualWidth}px`);
  root.style.setProperty("--app-height",`${virtualHeight}px`);
  root.style.setProperty("--responsive-font-boost",`${fontBoost}px`);
  root.style.setProperty("--compact-viewport-height",`${virtualHeight}px`);
  setPx("--ai-size",50);
  ...
  document.querySelectorAll("iframe").forEach(frame=>{
    try{
      ...
      if(doc?.documentElement){
        ...
        doc.documentElement.style.setProperty("--responsive-font-boost",`${fontBoost}px`);
        doc.documentElement.style.setProperty("--compact-viewport-height",`${virtualHeight}px`);
...
function bindResponsiveFrameSync(){
  document.querySelectorAll("iframe").forEach(frame=>{
    if(frame.dataset.caioResponsiveSync==="1")return;
    frame.dataset.caioResponsiveSync="1";
    frame.addEventListener("load",()=>{
      requestAnimationFrame(()=>{
        syncResponsiveScale();
        syncDashboardFrameActivity();
      });
    });
  });
}
