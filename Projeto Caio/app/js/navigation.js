...
function syncResponsiveScale(){
  ...
  const virtualWidth=nextScale<.999 ? baseWidth : viewportWidth;
  const virtualHeight=nextScale<.999 ? baseHeight : viewportHeight;
  const fontBoost=0;
  ...
  document.body.classList.toggle("is-app-scaled",nextScale<.999);
  document.body.classList.remove("is-responsive-scaled");
  document.querySelectorAll("iframe").forEach(frame=>{
    try{
      ...
      if(doc?.documentElement){
        ...
        doc.body?.classList.remove("is-responsive-scaled");
      }
    }catch(_){}
    try{
