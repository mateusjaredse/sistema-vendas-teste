(function(){
  ...
  function applyResponsiveScale(scale){
    const numeric = Number(scale);
    if(!Number.isFinite(numeric) || numeric <= 0) return;
    const fontBoost = numeric < .999 ? (numeric < .76 ? 2.6 : 1.6) : 0;
    document.documentElement.style.setProperty("--app-scale",numeric.toFixed(4));
    document.documentElement.style.setProperty("--responsive-font-boost",`${fontBoost}px`);
    document.body?.classList.toggle("is-responsive-scaled",numeric < .999);
  }
  ...
  window.addEventListener("message",event=>{
    ...
    if(event.data?.type==="caio-responsive-scale")applyResponsiveScale(event.data.scale);
  });
  ...
  function currentYear(){
    return brasiliaToday().getFullYear();
  }
