...
function workdayElapsedRatio(date){
  const now = new Date();
  if(date.toDateString() !== now.toDateString()) return 1;
...
function updateTodayCard(dayData,rows){
  ...
  if(metaFict) metaFict.textContent = "-R$ " + fmt(META_DIARIA);
  if(metaReal) metaReal.textContent = "-R$ " + fmt(META_REAL);
...
async function hydratePainelGeralFromSupabase(){
  ...
  const allMetaReal = metaRealRows.map(api.normalizeMetaReal).filter(row=>row.date);
  ...
  META_REAL = latestMeta?.metaRealDia || META_DIARIA;
