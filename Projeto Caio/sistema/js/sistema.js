...
let saleDateTimeAutoValue=null;
function setSaleDateTimeToBrasiliaNow(){
  const now=getBrasiliaDateTime();
  const dataInput=document.getElementById("v-data");
  const horaInput=document.getElementById("v-hora");
  if(dataInput)dataInput.value=now.date;
  if(horaInput)horaInput.value=now.time;
  saleDateTimeAutoValue=now;
}
function refreshSaleDateTimeIfAutomatic(){
  const dataInput=document.getElementById("v-data");
  const horaInput=document.getElementById("v-hora");
  if(!dataInput||!horaInput)return;
  const wasAutomatic=!saleDateTimeAutoValue
    || (!dataInput.value&&!horaInput.value)
    || (dataInput.value===saleDateTimeAutoValue.date&&horaInput.value===saleDateTimeAutoValue.time);
  if(wasAutomatic)setSaleDateTimeToBrasiliaNow();
}
...
window.onload=async()=>{
  ...
  refreshSaleDateTimeIfAutomatic();
  window.setInterval(refreshSaleDateTimeIfAutomatic,30000);
  window.addEventListener("focus",refreshSaleDateTimeIfAutomatic);
  document.addEventListener("visibilitychange",()=>{
    if(document.visibilityState!=="hidden")refreshSaleDateTimeIfAutomatic();
  });
  initSaleFileImport();
  addItem();
...
function notifySaleImportLocked(){
  toast("Importação de arquivos temporariamente bloqueada por segurança.","error");
}
...
function addSaleImportFiles(files){
  if(SALE_IMPORT_LOCKED){
    notifySaleImportLocked();
    return;
  }
...
function initSaleFileImport(){
  ...
  zone.addEventListener("click",()=>{
    if(SALE_IMPORT_LOCKED){notifySaleImportLocked();return;}
    input.click();
  });
...
async function importarArquivosSecundarios(files){
  if(SALE_IMPORT_LOCKED){
    throw new Error("Importação de arquivos temporariamente bloqueada por segurança.");
  }
  let inseridos=0;
  let avisos=0;
  avisosImportacao=[];
...
async function salvarVenda(){
  ...
  if(temArquivo&&SALE_IMPORT_LOCKED){
    notifySaleImportLocked();
    return;
  }
